...
Provocative critique of short-timeline AGI predictions: if we're close to human-like learners, then current RL scaling approaches are fundamentally misguided. The core observation: "Human workers are valuable precisely because we don't need to build in the bespoke training loops for every single small part of their job." Models lack on-the-job learning, which is why economic impact lags capability benchmarks. Economic diffusion lag is "cope"—if these were true AGI, they'd integrate faster than human hires. Prediction: labs will make progress on continual learning by 2030 but won't automate all knowledge work.
