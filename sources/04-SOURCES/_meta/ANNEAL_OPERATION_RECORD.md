...
**Objective:** Resolve all cross-pool duplicates, classify every file, and relocate non-source artifacts out of sources/.
...
**Objective:** Rename all source files to canonical naming convention, inject/complete YAML frontmatter, flatten all subdirectories, and merge all three pools into a single flat sources/ root.
...
- Final merge agent (move Pool A renamed files into sources/)
...
- Pool C: 101 sources renamed + moved, 52 research streams to praxis, 10 meta files, 4 assets, 15 non-source artifacts
- Pool A: 449 files renamed to SOURCE- prefix, 447 moved to sources/ (2 collisions resolved)
- 2 collision files resolved (kept larger version from sources/)
...
**Final state:** 627 SOURCE-*.md files in flat sources/ root. Zero files in subdirectories (except `_meta/`, `_config/`, `_index/`, `_assets/`).
...
4. **Keeper path mismatches.** 32 Pool C files could not be safely deleted during dedup because their designated keeper's path pointed to `sources/research/<filename>.md` -- a path that only existed before subdirectory flattening. These required resolution in Pass 2 after paths stabilized.
...
All artifacts produced during this operation, in `sources/_meta/`:
