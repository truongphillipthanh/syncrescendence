...
Do NOT include any explanation, commentary, or markdown code fences. Output the raw SN file only.
"""
...
def compress_with_llm(skeleton, canon_source, model="gpt-4o-mini", dry_run=False):
    """Send to LLM API for semantic compression. Supports OpenAI and Anthropic models."""
    if dry_run:
...
def main():
    ...
    print(f"V-4 PoC: Semantic Compression {'[DRY RUN]' if dry_run else ''}")
    print(f"Model: {model}")
    print(f"Files: {len(POC_FILES)}")
    print(f"Output: {OUTPUT_DIR}")
    print("=" * 60)

    for canon_id in POC_FILES:
        print(f"\n→ {canon_id}...")
        ...
        t0 = time.time()
        try:
            compressed = compress_with_llm(skeleton, canon_source, model=model, dry_run=dry_run)
