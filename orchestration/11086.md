...
The repo at HEAD (8f412bf, proxying the intent of 93a08409 with recent centralization commits) shows classic self-similar duplication: root-level prefixed vaults (orchestration/, 04-SOURCES/, 05-SIGMA/, 01-CANON/) mirror content inside orchestration/, engine/, praxis/, canon/. This is not noise; it is the memory lattice doing its job until now. The map below collapses it ruthlessly to the agreed four directories while preserving every living atom of the five Rust traits. Every path is walked; none left ambiguous.

orchestration/ → scaffold/orchestration-legacy/ (operational coordination core; temporary holding for review then prune)
orchestration/orchestration/ → scaffold/ (strategic templates, launchd, scripts—dissolves into scaffold sub-buckets)
orchestration/orchestration/archive/ → logs/archive/ (temporal historical records)
orchestration/orchestration/state/ → logs/state/ (active execution staging)
orchestration/orchestration/scripts/ → scaffold/scripts/
orchestration/orchestration/templates/ → scaffold/templates/
...
orchestration/orchestration/state/DYN-*.md → logs/DYN-*.md (state machine writes, every dispatch)
orchestration/orchestration/state/*.json → logs/state/ (snapshots, per-session)
...
launchd plists / tmux configs referenced in orchestration/orchestration/ → scaffold/configs/launchd/ (operational)
...
git mv orchestration/orchestration/state logs/state/ 2>/dev/null || true
git mv orchestration/orchestration/archive logs/archive/ 2>/dev/null || true
...
git mv orchestration/orchestration/templates scaffold/templates/ 2>/dev/null || true
git mv orchestration/orchestration/scripts scaffold/scripts/ 2>/dev/null || true
...
sed -i '' 's|orchestration/orchestration/|scaffold/|g' AGENTS.md Makefile 2>/dev/null || true
