...
STAGING="$REPO_ROOT/orchestration/state/DYN-EXECUTION_STAGING.md"
...
if [ "$ENTRY_COUNT" -ge 10 ]; then
    ...
    bash "$REPO_ROOT/orchestration/scripts/compact_wisdom.sh"
    # Stage and commit the compaction
    git add "$STAGING" \
        "$REPO_ROOT/orchestration/archive/ARCH-EXECUTION_HISTORY.md" \
        "$REPO_ROOT/orchestration/archive/ARCH-DIRECTIVE_COMPENDIUM.md" \
        2>/dev/null
