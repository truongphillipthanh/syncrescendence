...
if [ -x "orchestration/scripts/verify_orchestration_hardening.sh" ]; then
    if bash "orchestration/scripts/verify_orchestration_hardening.sh"; then
...
if [ -f "orchestration/state/DYN-TASKS.csv" ]; then
    TASKS=$(wc -l < orchestration/state/DYN-TASKS.csv | tr -d ' ')
    DONE=$(grep -c ',done,' orchestration/state/DYN-TASKS.csv 2>/dev/null || echo 0)
...
if [ -f "orchestration/state/DYN-PROJECTS.csv" ]; then
    PROJECTS=$(wc -l < orchestration/state/DYN-PROJECTS.csv | tr -d ' ')
    COMPLETE=$(grep -c ',complete,' orchestration/state/DYN-PROJECTS.csv 2>/dev/null || echo 0)
...
if [ -f "sources/DYN-SOURCES.csv" ]; then
    SOURCES=$(wc -l < sources/DYN-SOURCES.csv | tr -d ' ')
    PROCESSED=$(grep -c ',processed,' sources/DYN-SOURCES.csv 2>/dev/null || echo 0)
...
echo -n "| CANON files: "
CANON=$(ls canon/*.md 2>/dev/null | wc -l | tr -d ' ')
echo "$CANON"

echo -n "| Processed sources: "
PROC_SOURCES=$(ls sources/processed/*.md 2>/dev/null | wc -l | tr -d ' ')
echo "$PROC_SOURCES"

echo -n "| CANON with integrations: "
INTEGRATIONS=$(grep -l "SOURCE-" canon/*.md 2>/dev/null | wc -l | tr -d ' ')
echo "$INTEGRATIONS"

echo -n "| Function XMLs: "
FUNCTIONS=$(ls engine/*.xml 2>/dev/null | wc -l | tr -d ' ' || echo "0")
echo "$FUNCTIONS"
echo "------------------------------------------"
echo ""
