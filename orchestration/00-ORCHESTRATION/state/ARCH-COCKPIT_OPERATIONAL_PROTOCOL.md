...
- Update memory architecture (`praxis/`) with session learnings
...
PROACTIVE:
  ...
  Update praxis/ with session learnings
...
DOCUMENT:
  Write RESULT-* files back to Commander's inbox
  Append findings to relevant praxis/ files
...
PROACTIVE:
  ...
  HOOKS: {/updateMemoryArchitecture — push learnings to praxis}  [ASPIRATIONAL]
...
| `/updateMemoryArchitecture` | All | Push session learnings to praxis | P2 |
...
| Terminal Stack Config | `orchestration/TERMINAL-STACK-CONFIG.md` |
| Fleet Commander's Handbook | `engine/REF-FLEET_COMMANDERS_HANDBOOK.md` |
| Twin Coordination Protocol | `orchestration/state/DYN-TWIN_COORDINATION_PROTOCOL.md` |
