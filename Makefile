...
acumen-build-verification-bridge:
	python3 operators/acumen/build_verification_bridge.py --triage-ledger "$(or $(TRIAGE_LEDGER),orchestration/state/registry/acumen-triage-decision-ledger.jsonl)" --training-ledger "$(or $(TRAINING_LEDGER),orchestration/state/registry/acumen-training-corpus-ledger.jsonl)" --dossier-dir "$(or $(DOSSIER_DIR),runtime/acumen/out/verification-dossiers)" --bridge-status-json "$(or $(BRIDGE_STATUS_JSON),orchestration/state/ACUMEN-AUGUR-VERIFICATION-BRIDGE.json)" $(foreach DECISION,$(DECISIONS),--decision "$(DECISION)") $(foreach VIDEO_ID,$(VIDEO_IDS),--video-id "$(VIDEO_ID)") $(foreach EVENT_ID,$(TRIAGE_EVENT_IDS),--triage-event-id "$(EVENT_ID)")
...
acumen-live-batch:
	@test -n "$$ACUMEN_YOUTUBE_API_KEY" || (echo "blocked: missing ACUMEN_YOUTUBE_API_KEY in environment" && exit 2)
	@test -n "$$GEMINI_API_KEY" || (echo "blocked: missing GEMINI_API_KEY in environment" && exit 2)
	$(MAKE) acumen-validate-registry REGISTRY="$(or $(REGISTRY),runtime/acumen/registry.json)" STRICT=1
	$(MAKE) acumen-identity-probe BINDING="$(or $(BINDING),orchestration/state/ACUMEN-IDENTITY-BINDING-CC87.json)" OUTPUT="$(or $(IDENTITY_OUTPUT),orchestration/state/ACUMEN-IDENTITY-STATUS.json)" STRICT=1
	$(MAKE) acumen-pipeline-run QUEUE="$(or $(QUEUE),runtime/acumen/triage-decisions.jsonl)" OUT="$(or $(OUT),runtime/acumen/out)" REGISTRY="$(or $(REGISTRY),runtime/acumen/registry.json)" STATUS_JSON="$(or $(STATUS_JSON),orchestration/state/ACUMEN-PIPELINE-STATUS.json)" BINDING="$(or $(BINDING),orchestration/state/ACUMEN-IDENTITY-BINDING-CC87.json)" IDENTITY_STATUS_JSON="$(or $(IDENTITY_OUTPUT),orchestration/state/ACUMEN-IDENTITY-STATUS.json)" POLL_STATUS_JSON="$(or $(POLL_STATUS_JSON),orchestration/state/ACUMEN-YOUTUBE-POLL-STATUS.json)" TRIAGE_STATUS_JSON="$(or $(TRIAGE_STATUS_JSON),runtime/acumen/triage-status.json)" POLL_MODE=live TRIAGE_MODE=gemini FORCE_POLL="$(or $(FORCE_POLL),1)" TRAINING_JSONL="$(or $(TRAINING_JSONL),runtime/acumen/training-corpus.jsonl)" TRIAGE_ARTIFACT_DIR="$(or $(TRIAGE_ARTIFACT_DIR),runtime/acumen/out/triage)" API_KEY_ENV="$(or $(API_KEY_ENV),GEMINI_API_KEY)" TIMEOUT_SECONDS="$(or $(TIMEOUT_SECONDS),45)" MAX_LIVE_CALLS="$(or $(MAX_LIVE_CALLS),12)" MAX_RETRIES="$(or $(MAX_RETRIES),2)" RETRY_BACKOFF_SECONDS="$(or $(RETRY_BACKOFF_SECONDS),1.5)" MAX_PROMPT_CHARS="$(or $(MAX_PROMPT_CHARS),18000)" MAX_OUTPUT_TOKENS="$(or $(MAX_OUTPUT_TOKENS),220)" MAX_TOTAL_TOKENS="$(or $(MAX_TOTAL_TOKENS),4096)" ESTIMATED_COST_PER_CALL_USD="$(or $(ESTIMATED_COST_PER_CALL_USD),0)" MAX_ESTIMATED_COST_USD="$(or $(MAX_ESTIMATED_COST_USD),0)" STRICT_IDENTITY=1
