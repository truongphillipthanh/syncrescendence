...
├── canon/ (all files)
├── orchestration/state/ (key reference files)
...
```json
{
  ...
  "corpus_slice": [
    "Files consulted (e.g., canon/[[CANON-00004-EVOLUTION-cosmos]].md)",
...
Evidence Packet:
{
  ...
  "corpus_slice": [
    "orchestration/state/REF-STANDARDS.md",
    "Oracle-07 (FrontierModels.md deletion discussion)",
    "canon/CANON-00001-PRINCIPLES.md"
  ],
  "findings": [
...
Evidence Packet:
{
  ...
  "findings": [
    ...
    "5 commits to orchestration/state/",
    "PROJ-002 status updated (60% → 100%)"
  ],
  "uncertainties": [
    "No commits to canon/ this week (expected?)"
  ],
...
```
// BAD (not verified)
"See file engine/nonexistent.md"

// GOOD (verified)
"`ls engine/` shows no file matching that name. Did you mean IIC-Acumen-config.md?"
```
...
```json
{
  "findings": [
    ...
    "engine/IIC-Coherence-config.md:line-89 - 'Opus recommended for framework development'"
  ],
  "uncertainties": [
