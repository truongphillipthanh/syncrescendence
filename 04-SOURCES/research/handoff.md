...
**Stage 1: Capture** - Research artifacts land in `04-SOURCES/raw/deep-research/` with timestamped filenames and source metadata.
...
**Stage 4: Integration** - Validated content moves to `01-CANON/` with taxonomy classification. Create index files (`corpus_index.md`) with searchable summaries enabling Claude to navigate without loading full documents.

Custom slash commands systematize this workflow:

```markdown
# .claude/commands/metabolize-research.md
Read the specified research artifact in 04-SOURCES/raw/deep-research/.
Extract key findings, validate against existing canonical documents.
Create condensed knowledge file (<50KB) in appropriate 01-CANON/ taxonomy location.
...
## Quick Reference
- **Corpus**: 75 canonical docs in 01-CANON/, 185 sources in 04-SOURCES/
...
@00-ORCHESTRATION/state/current.md for system state
...
│  ├── 01-CANON/ through 06-EXEMPLA/  ← Your structure       │
│  └── ledgers/                                               │
