...
(Description: A terminal task dashboard showing 4 tasks: 2 completed, 2 in progress, with 16 open tasks total. Tasks #4 and #5 are blocked by #3. Navigation hint: ctrl+t to hide tasks)
...
(Description: JSON schema showing TaskUpdate tool with taskId, status, owner, and addBlockedBy fields demonstrating how to modify existing tasks)
...
(Description: A JSON schema showing the complete structure of a stored task with id, subject, description, activeForm, owner, status, blocks, blockedBy, and metadata fields)

### Accessing Task Files

You can inspect, backup, or even manually edit these files:

- **View all task lists** > `ls ~/.claude/tasks/`
- **View a specific task list** > `ls ~/.claude/tasks/my-project/`
- **Read a specific task** > `cat ~/.claude/tasks/my-project/1.json | jq`
...
(Description: Diagram showing 3 sequential phases: PHASE 1: IMPLEMENTATION with task "1. Add logout button to navbar component" (Add button with onClick handler in NavBar.tsx), PHASE 2: IMPLEMENTATION with task "2. Implement logout API call" (Clear session, revoke token, redirect to login) with BLOCKED BY: 1, and PHASE 3: TESTING with task "3. Test logout flow" (Verify session cleared, redirect works, edge cases handled) with BLOCKED BY: 2. Boxes are connected with arrows showing linear progression)
...
{
  "subject": "Design JWT implementation plan",
  "addBlockedBy": ["1", "2"]
}
...
{
  "subject": "Update all protected routes",
  "addBlockedBy": ["4"]
}
{
  "subject": "Add token refresh mechanism",
  "addBlockedBy": ["4"]
}
...
(Description: Diagram showing 4 phases: PHASE 1: INVESTIGATION (tasks 1 and 2 running in parallel - "1. INVESTIGATE CURRENT SESSION IMPLEMENTATION" and "2. RESEARCH JWT BEST PRACTICES"), PHASE 2: PLANNING (task 3 "DESIGN JWT IMPLEMENTATION PLAN" blocked by 1, 2), PHASE 3: IMPLEMENTATION (tasks 4, 5, 6 with task 4 "IMPLEMENT JWT AUTHENTICATION" blocked by 3, tasks 5 and 6 blocked by 4), PHASE 4: TESTING (task 7 "WRITE INTEGRATION TESTS" blocked by 5, 6). Blue arrows show dependencies between phases)
...
{
  "subject": "Confirm final headcount",
  "addBlockedBy": ["6"],
  "owner": "planner"
}
```

(Description: Diagram with 4 phases: PHASE 1: FORMATION, PHASE 2: PLANNING & BOOKING, PHASE 3: INVITATIONS & RSVPS, PHASE 4: FINALIZATION. Tasks shown: 1. BOOK VENUE (owner: couple), 2. CREATE GUEST LIST (owner: couple), 3. SET WEDDING DATE (owner: couple, blocked by 1), 4. BOOK CATERER (owner: planner, blocked by 1, 2), 5. SEND INVITATIONS (owner: couple, blocked by 2, 3), 6. COLLECT RSVPS (owner: couple, blocked by 5), 7. FINALIZE SEATING CHART (owner: planner, blocked by 4, 6), 8. CONFIRM FINAL HEADCOUNT (owner: planner, blocked by 6). Multiple parallel paths shown with arrows connecting dependent tasks)
...
You can utilise the CLAUDE_CODE_TASK_LIST_ID to group all tasks under. This is not tied to the current directory you're in, it will be used to manage all tasks as long as it's specified. There are two ways to do this:
...
6. Check progress anytime - Press ctrl+t to toggle the task view, or ask "what's the task status?"
