...
(Image: Instruction-following performance chart showing 16 different LLM models. The chart uses a 4x4 grid layout displaying accuracy (%) on the y-axis and number of instructions on the x-axis. Each subplot shows how instruction-following accuracy decreases as the number of instructions increases from 0 to 500. Models include gemini-2.5-pro-preview, o3, grok-3-beta, claude-3.7-sonnet, gpt-4.1, claude-opus-4, gpt-4.5-preview, claude-sonnet-4, grok-3-mini-beta, o4-mini, gemini-2.5-flash-preview, deepseck-r1, llama-4-maverick, gpt-4.1-mini, qweng-235b, gpt-4.1-nano, gpt-40, gpt-40-mini, claude-3.5-haiku, llama-4-scout. A color scale on the right indicates latency ($) ranging from 20 to 120. The chart is sourced from https://arxiv.org/pdf/2507.11538)
...
If you feel very strongly about this, you might even consider setting up a [Claude Code Stop hook](https://code.claude.com/docs/en/hooks#stop) that runs your formatter & linter and presents errors to Claude for it to fix. Don't make Claude find the formatting issues itself.
...
Both Claude Code and other harnesses with OpenCode come with ways to auto-generate your `CLAUDE.md` file (or `/init` and `AGENTS.md`).
...
(Image: Hierarchy of Leverage diagram showing a pyramid structure with five levels. From bottom to top:
1. Bottom level (lightest): "1 bad line of code = 1 bad line of code"
2. Second level: "1 bad line of plan = 100 wrong lines of code → WRONG SOLUTION"
3. Third level: "1 bad line of research = 10-100 wrong lines of plan → WRONG UNDERSTANDING OF SYSTEM"
4. Fourth level: "1 bad line of specification = a 10-100 wrong lines of research → WRONG UNDERSTANDING OF PROBLEM"
5. Top level (darkest blue): "1 bad line of CLAUDE.md affects every single spec, research and plan → DEGRADATION OF CORE COMPETENCY AND METHODOLOGY"

The left side includes a QR code and attribution text reading "How to Write a Good CLAUDE.md @0xblacklight & @dexwonthy". Arrows on the right point from upper levels to lower levels, showing cascading impact. A legend on the right indicates the color scale represents multiplication of impact.)
