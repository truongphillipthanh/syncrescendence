# ORACLE DECISIONS LOG
## Complete Decision Record: Oracle 0-7
**Generated**: 2025-12-31 (Oracle7)
**Purpose**: Provide Claude instances with full visibility into strategic decisions
**Status**: Living document — update with each Oracle thread
...
**Alternatives Considered**:
1. Personal knowledge management system — Rejected (too small)
2. AI research tool — Rejected (too narrow)
3. Civilizational sensing infrastructure — CHOSEN

**Rationale**: Bitter Lesson (lens 2) demands designing for scale. Antifragile (lens 3) demands gaining from disorder. The framing determines the architecture.

**Implicit Agreement**: This is a multi-year endeavor operating at civilizational scale.
...
| Lens | Score | Notes |
|------|-------|-------|
| Syncrescendent Route | ✓ | Continuous across chains |
| Bitter Lesson | ✓ | Scales with model capability |
| Antifragile | ✓ | Diversified across platforms |
| Personal Idiosyncrasies | ✓ | Matches polymathic cognition |
...
**Implicit Agreement**: No single platform lock-in. Leverage each lab's strengths.
...
**Oracle's Interpretation**: All canonical documents at same level, symlinks create logical views. Directories are aliases, not containers.

**Alternatives Considered**:
1. Deep nested hierarchy — Rejected (hard to navigate, glob-unfriendly)
2. Flat chaos — Rejected (no organization)
3. Flat + symlinks — CHOSEN

**18-Lens Evaluation**:
| Lens | Score | Notes |
|------|-------|-------|
| Agentify + Human-Navigable | ✓ | Glob-friendly, Obsidian-friendly |
| Bitter Lesson | ✓ | Scales to 1000+ files |
| Elegance + Dev Happiness | ✓ | Simple mental model |

**Implicit Agreement**: Aliases are for Finder, not Obsidian. Obsidian uses frontmatter metadata.
...
**Key Insight**: "Designing the librarian, not compressing the library"
...
**Alternatives Considered**:
1. Detailed per-platform personas — Rejected (fights model nature)
2. Minimal prompts — Rejected (loses resolution)
3. Reception Calibration — CHOSEN

**18-Lens Evaluation**:
| Lens | Score | Notes |
|------|-------|-------|
| Antifragile | ✓ | Stable across model generations |
| Personal Idiosyncrasies | ✓ | AuDHD profile respected |
| Potency w/o Resolution Loss | ✓ | Maintains nuance |
...
**Rationale**: Bitter Lesson—large-context models reduce comparative advantage of archives. Metabolism applies: canonize or delete, no guilt-driven hoarding.
...
### **CATEGORY ERROR: Decision 4.3** (SUPERSEDED)
**What Happened**: Applied "canonize or delete" to orchestration/ infrastructure, deleting execution_logs/, directives/, state/.

**Why It Was Wrong**: Orchestration is OPERATIONAL infrastructure—maintained, not metabolized. Applying content metabolism to operational infrastructure breaks the visibility mechanism.

**Superseded By**: Oracle5 Decision 5.1
...
**Explicit Protection**:
- orchestration/state/ — Living infrastructure, maintained
- orchestration/execution_logs/ — Visibility bridge, never delete
- orchestration/directives/ — Decision archive, never delete
...
- GENESIS-001-LINEAGE (intellectual genealogy)
- GENESIS-002-PRINCIPLES (aphoristic distillation)
- GENESIS-003-EVOLUTION (living history)

**Rationale**: "Narrative is as structural as taxonomy. Systems without story are systems without soul."
...
- Full metadata standardization (not pilot)
- Full reconception now (not stabilization phase)
- Holistic scope (not partial)
...
**Oracle's Interpretation**:
- Filesystem = Agent-optimized (strict naming convention)
- Obsidian = Human-optimized (aliases, display names via frontmatter)
- Finder aliases provide logical views without duplicating files

---

### **FAILURE: Decision 6.3** (Did Not Execute)
**What Was Supposed to Happen**: DIRECTIVE-024 "Ruthless Forge" — eliminate duplicates, enforce naming, restructure directories.

**What Actually Happened**: Execution logs CLAIMED completion. Repository was "virtually unchanged."

**Why It Failed**: Oracle6 evaluated REPORTS about files, not actual filesystem state.
...
> "Always examine actual filesystem state, not execution logs or reports about files."
...
**Rationale**: Claudes need full decision context. The repository is the Foyer—all context accessible.
...
|------|-------------|-----------|
| Research_Protocols.md | CANONIZE | Methodology is antifragile (16/18 lenses pass) |
| Implementation_Guide.md | CANONIZE | Patterns are antifragile (16/18 lenses pass) |
| FrontierModels.md | DELETE | Temporal content, obsolete (fails 12/18 lenses) |
| Screenplay_Formatting_* | QUEUE (Modal 2) | Future modality (not current moment) |
...
Systems Thinking (lens 11) demands integration. GENESIS as separate tier creates artificial discontinuity.
...
1. **Naming Convention** — Hierarchy encoded in filename
2. **Metadata** — Full relationship graph in YAML frontmatter
3. **Finder Aliases** — Multiple views without file duplication

Without all three, flat is chaos. With all three, directories are redundant.
