...
4. **Also update** `orchestration/scripts/launchd-psyche/com.syncrescendence.watch-cartographer.plist` with the same changes (this template also references `/Users/system` paths but should use `/Users/home` for Mac mini -- note it currently has `/Users/system` paths which is wrong for Mac mini).
