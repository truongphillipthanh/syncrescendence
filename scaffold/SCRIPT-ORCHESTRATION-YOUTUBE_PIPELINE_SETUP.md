...
Stateless YouTube ingest pipeline: Liked Videos become SOURCE-*.md files in `sources/`, then get removed from the playlist. Inbox zero for YouTube saves.
...
```bash
python orchestration/scripts/youtube_oauth_setup.py
```
...
```bash
python orchestration/scripts/build_url_index.py
```

This scans existing SOURCE-*.md files and creates `sources/_meta/URL_INDEX.txt` — the dedup gate that prevents re-ingesting videos already in the corpus.

## 6. Run the Ingest

### Dry run (see what would be ingested):
```bash
python orchestration/scripts/youtube_ingest.py --dry-run
```

### Ingest Liked Videos (default):
```bash
python orchestration/scripts/youtube_ingest.py
```

### Ingest and clear from Liked after success:
```bash
python orchestration/scripts/youtube_ingest.py --clear-after-ingest
```

### Ingest from a specific playlist:
```bash
python orchestration/scripts/youtube_ingest.py --playlist PLxxxxxxxxxxxxxxxx
```

### Ingest from multiple playlists:
```bash
python orchestration/scripts/youtube_ingest.py --playlist LL --additional-playlists PLaaa PLbbb
```
...
```bash
cp orchestration/scripts/com.syncrescendence.youtube-ingest.plist ~/Library/LaunchAgents/
...
| URL dedup index | `sources/_meta/URL_INDEX.txt` |
