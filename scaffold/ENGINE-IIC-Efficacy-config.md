...
grep -n "CANON-XXXXX" canon/target.md

# Ledger updated
grep "PROJ-XXX" orchestration/state/tasks.csv
```
...
```bash
# 1. Ingest source
cp /path/to/source.md sources/raw/
...
# 4. Verify
grep "SOURCE-YYYYMMDD-NNN" orchestration/state/sources.csv
```

**Pattern: CANON Integration**
```bash
# 1. Read target CANON
claude read canon/CANON-XXXXX.md

# 2. Identify insertion point
grep -n "## Relevant Section" canon/CANON-XXXXX.md

# 3. Edit with citation
claude edit canon/CANON-XXXXX.md \
  --insert-at="line_number" \
  --cite="SOURCE-YYYYMMDD-NNN"

# 4. Verify integration
grep "SOURCE-YYYYMMDD-NNN" canon/CANON-XXXXX.md
```
...
**Location**: `engine/efficacy-memory/`
...
ls -la sources/raw/sutton-interview.md
# ✓ 450KB

# 2. Apply processing function
python3 engine/functions/transcribe_interview.py \
  sources/raw/sutton-interview.md \
...
ls -la sources/processed/SOURCE-20250926-*.md
# ✓ File exists

grep "SOURCE-20250926-057" orchestration/state/sources.csv
# ✓ status=processed

grep "^topics:" sources/processed/SOURCE-20250926-*.md
...
```json
{
  ...
  "deliverables": [
    "sources/processed/SOURCE-20250926-youtube-interview-dwarkesh_patel-richard_sutton.md",
    "orchestration/state/sources.csv (updated)"
  ],
  "verification": {
