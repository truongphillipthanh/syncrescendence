...
### Structural (ABSOLUTE)
1. **FLAT PRINCIPLE**: All directories must be flat. Use naming prefixes (ARCH-, DYN-, REF-, SCAFF-, FUNC-, PROMPT-, etc.) instead of subdirectories. Sanctioned exceptions: `praxis/mechanics/`, `practice/`; `orchestration/state/`, `scripts/`, `archive/`; `agents/<name>/` internal structure.
...
| Semantic Notation | `orchestration/scripts/SN_BLOCK_TEMPLATES.md`, `sn_symbols.yaml` |
| SN encoding/decoding | `orchestration/scripts/sn_encode.py`, `sn_decode.py` |
...
3. `orchestration/scripts/auto_ingest_loop.sh` polls INBOX0 every 30s
...
# From MBA (remote):
ssh mini "cat ~/Desktop/syncrescendence/orchestration/state/DYN-CONSTELLATION_HEALTH.md"
```
...
```bash
bash orchestration/scripts/dispatch.sh <agent> "TOPIC" "DESC" "" "TASK" "commander"
```
