...
gemini --model gemini-2.5-pro "Survey all files in canon/ for structural inconsistencies"

# With settings file:
gemini --settings engine/gemini-settings.json "analyze corpus"

# Stateless evidence pack generation:
gemini --model gemini-3-flash "Generate token count inventory for sources/"
```

**Config**: `engine/gemini-settings.json`, context file: `engine/AVATAR-GEMINI-CLI.md`
...
gemini "survey canon/ using analysis from analysis.md"
...
- `engine/gemini-settings.json` — Gemini CLI settings template
- `orchestration/state/DYN-TWIN_COORDINATION_PROTOCOL.md` — Twin coordination
- `engine/DYN-COORDINATION.yaml` — Platform coordination state
