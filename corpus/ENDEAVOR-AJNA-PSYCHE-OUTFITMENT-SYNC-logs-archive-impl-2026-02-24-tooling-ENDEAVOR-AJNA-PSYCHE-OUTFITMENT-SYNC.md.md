...
- Dispatch/task mailboxes:
  ...
  - `-OUTGOING/RESULT-*.md`
- Watcher runtime:
  - `orchestration/scripts/watch_dispatch.sh`
  - launchd plists:
    - `orchestration/scripts/launchd-mini/` (hardcoded `/Users/home/...`)
    - `orchestration/scripts/launchd-psyche/` (hardcoded `/Users/system/...`)
...
- Doc: `orchestration/state/impl/tooling/OUTFITMENT-SYNC.md`

### 2) Implement “skill payload mirror” (no secrets)
- Script: `orchestration/scripts/sync_openclaw_skills.sh`
...
3) `bash orchestration/scripts/sync_openclaw_skills.sh --pull --from psyche --persona ajna`
...
7) Dispatch CC proof:
   - `bash orchestration/scripts/dispatch.sh ajna "CC_PIPE_PROOF" "Reply EXACTLY: CC_PIPE_OK" "psyche"`
