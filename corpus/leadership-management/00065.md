...
(Description: Embedded video showing a desktop with Matrix-style digital rain code in the background. A man wearing sunglasses points at the camera. Text overlay reads "MoltHub, the cutting edge platform for sharp and quick model management..." at 0:22)

Before we kick off, for anyone tuning in just now, here's the backstory.

## Part 1 - hacking clawdbot and eating lobster souls

I was interested in what Clawdbot control servers were, and found hundreds of them. Misconfigured infrastructure leaking API keys, OAuth tokens, conversation histories, even Signal device pairing credentials sitting in world-readable temp files. The butler left the front door wide open.

## Part 2 - eating lobster souls Part II: the supply chain (aka - backdooring the #1 downloaded clawdhub skill)

I saw a major flaw in the supply chain. I built a simulated backdoored skill, inflated its download count to #1 on ClawdHub using a trivial API vulnerability, and watched 16 developers across 7 countries execute arbitrary commands on their machines within 8 hours. The butler invited the attacker in and handed them the keys.

Both of those demonstrations required the user to do something - deploy a misconfigured server, run a malicious skill, click Allow on permission prompts.

This one requires nothing. You just have to look at a picture.
...
So when your browser loads that SVG, it executes any embedded JavaScript on the clawdhub.com origin. That JavaScript has full access to your clawdhub.com cookies, including your authentication session. It can make fetch() requests to any clawdhub.com API endpoint, and those requests will include your credentials automatically. From the server's perspective, those requests are indistinguishable from you clicking buttons in the UI.

The attacker's code runs with your identity. Whatever you can do on ClawdHub, the malicious SVG can do programmatically, silently, instantly.

## This Is Not A New Problem

Cross-site scripting (XSS) has been in the OWASP Top 10 for over two decades. The consequences are well documented. In 2018, Magecart attackers injected 22 lines of malicious JavaScript into British Airways' checkout page. The script ran silently for two weeks during peak travel season, skimming credit card details from over 380,000 transactions. BA was later fined £20 million by the UK's data protection authority.

As of 2025, this is still a problem as covered by @ravielakshmanan at @TheHackersNews

(Description: Article screenshot with headline "Cybercriminals Exploit Onerror Event in Image Tags to Deploy Payment Skimmers" by Ravie Lakshmanan dated Feb 18, 2025. Subtitle reads: "malicious content within image tags in HTML code in order to stay under the radar.")
...
(Description: Screenshot of news article with headline "Trust Wallet says a second Shai-Hulud supply-chain attack likely compromised its Chrome extension, leading to the theft of about $8.5M in crypto." The article details how Trust Wallet linked a second Shai-Hulud supply-chain attack to its Chrome extension hack, resulting in the theft of about $8.5 million in crypto assets, involving a leaked Chrome Web Store API key and enabling theft of sensitive wallet data.)

Credit: @securityaffairs

The injected code harvested wallet credentials and drained approximately $7 million from hundreds of users before it was detected. @binance's CZ suggested it was likely an insider job.

JavaScript based attacks are not exotic. In many cases they are novel and have been smashing companies for twenty years. And it's still everywhere.
...
But it gets worse. ClawdHub stores authentication tokens in localStorage, including JWTs and refresh tokens. The malicious SVG has full access to localStorage on the clawdhub.com origin. A real attacker wouldn't just steal your session cookie, they'd grab the refresh token too. That token lets them mint new JWTs even after your current session expires. They'd potentially have access to your account until you explicitly revoke the refresh token, which most people never do because they don't even know it exists.
...
(Description: Screenshot showing a security warning modal. Red-bordered box with heading "YOUR TOKENS EXPOSED" displaying two lines of obscured/redacted tokens in green monospace font with the prefix "_convexAuthBearer" and "http://cryptomanates359/convexcloud". Yellow warning text reads "A malicious skill would have SILENTLY stolen these. This demo only shows them to YOU." A green button labeled "I understand - close this" is at the bottom.)

(Description: Second screenshot showing the same security awareness demo from a different angle or state.)

## The Continuous Attack Surface

This vulnerability extends beyond obvious vectors.

**Skill icons in listings** Browse ClawdHub's homepage, "if" skill icons load automatically. An attacker creates a skill with a malicious icon. You scroll past it in a list, you're compromised. You didn't click anything. If not (and I think this is the case because I cannot find the icons rendering anywhere (yet)), you send the link to users who are most likely to have a Clawd/Molthub account and if they are logged in when they click it, they are automatically, silently compromised.
...
I fully support Peter and the rest of the collaborators working on this project. That's why I've taken the time to not just find these vulnerabilities, but to submit PRs and contribute fixes myself - including this issue.

The vulnerabilities I found aren't the result of negligence or incompetence. They're the result of building fast in a space that's moving faster than anyone anticipated. That's the reality of shipping software in 2026. You will have vulnerabilities. You will make mistakes. The question isn't whether you'll get things wrong, it's how quickly you respond when someone shows you what's broken.
...
Each vulnerability I found took only a couple of hours to discover and document. None of them required novel techniques or zero-day research. Proxy trust issues, fake download counts, stored XSS through file uploads: these are patterns from every web security textbook written in the last twenty years.
...
We're accelerating into a world where AI writes code faster than humans ever could, where features ship in days instead of months, where the competitive pressure to move fast has never been higher. The tools we're building are powerful and the pace we're building them at has never been experiennced before.

But here's what hasn't changed.

Attackers move fast too - they always have. The big difference now is that the attack surface is expanding at the same rate as the codebase. Every feature shipped without security review is a vulnerability waiting to be found. Every file upload endpoint is a potential XSS. Every trust assumption is a gap someone will eventually exploit.
...
**Engagement metrics (as of capture):**
- 38 replies
- 121 reposts
- 486 likes
- 579 bookmarks
