...
│  ┌─────────────────────────────────────────────────────┐    │
│  │  ~/syncrescendence/                                  │    │
│  │  ├── canon/          (canonical knowledge)        │    │
│  │  ├── engine/    (prompts, memory configs)    │    │
│  │  ├── sources/        (raw + processed sources)    │    │
│  │  └── orchestration/  (state, directives, logs)    │    │
│  └─────────────────────────────────────────────────────┘    │
│              │                │                │             │
│              ▼                ▼                ▼             │
│    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    │
...
|-----------|-------------|----------------|
| **Corpus Documents** | Canonical knowledge shared by all IICs | CANON files in canon/ |
| **Memory Configs** | Per-IIC platform configurations | engine/memory/*.md |
| **Thread Culminations** | Session summaries for context transfer | orchestration/logs/*.md |
| **Context Files** | Active context for handoffs | engine/context/*.md |
| **State Files** | Orchestration state | orchestration/state/*.md |
...
2. Bundle saved to sources/processed/
...
- [List of transcript files in sources/processed/]
```
...
**Transfer Method**:
1. Coherence produces framework in canon/
2. Efficacy-relevant implementation notes added to engine/
...
2. Pattern report saved to orchestration/state/
...
2. Proven procedures documented in engine/
...
4. If operational, update relevant engine/ document
5. If context-specific, save to orchestration/state/
