...
### 1. Architecture Doc
**File**: `orchestration/state/DYN-DISPATCH_KANBAN_PROTOCOL.md`
...
### 3. Migration Script
**File**: `orchestration/scripts/migrate_kanban.sh`
...
**File**: `orchestration/scripts/queue_status.sh`
...
2. **Test dispatch (creates task in 00-INBOX0)**:
   ```bash
   bash orchestration/scripts/dispatch.sh commander "SMOKE_TEST" "Echo hello world"
...
4. **Test queue status**:
   ```bash
   bash orchestration/scripts/queue_status.sh
   bash orchestration/scripts/queue_status.sh commander
   ```

5. **Verify watcher compatibility** (manual test on target machine):
   ```bash
   bash orchestration/scripts/watch_dispatch.sh commander
