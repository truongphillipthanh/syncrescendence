...
echo "Source: $REPO_ROOT/orchestration/state/impl/deploy/"
echo ""
...
scp "$REPO_ROOT/orchestration/state/impl/deploy/PSYCHE-SLACK-CONFIG.json" \
    "$MAC_MINI:~/.openclaw/openclaw.json.psyche-slack"

# Copy env template (Sovereign must edit with real tokens)
scp "$REPO_ROOT/orchestration/state/impl/deploy/psyche-slack-env-template.sh" \
...
scp "$REPO_ROOT/orchestration/scripts/slack_health_check.sh" \
