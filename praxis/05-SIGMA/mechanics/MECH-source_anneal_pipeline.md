...
**Derived From:** The Great Source Anneal operation record (`sources/_meta/ANNEAL_OPERATION_RECORD.md`)
...
Each stage is **idempotent** -- re-running a stage on already-processed files produces no change and no corruption. Each stage writes a manifest of its actions to `sources/_meta/` for auditability.
...
### Actions
1. Copy file to `sources/_staging/` (temporary holding area)
...
6. Move to `sources/` root
7. Log action to `_meta/INGEST_LOG.md`

### Implementation
- **Mechanism:** `fswatch` on macOS (or `launchd` periodic polling every 60s)
- **Script:** `sources/_scripts/ingest.py` (or `ingest.sh` for bash)
- **Trigger:** File creation in watch folders
- **Idempotency:** Skip files already present in `sources/` (match by URL or filename)
...
3. Write MOC files to `sources/_index/`
4. Write `DYN-SOURCES.csv` to `sources/_meta/`
...
- **Reference**: `praxis/syntheses/SYNTHESIS-gemini-cli.md` (Colab-to-Claude bridge pattern), `praxis/syntheses/SYNTHESIS-platform_topology_jan2026.md` (token economics)
...
anneal:
  source_dir: "sources/"
  meta_dir: "sources/_meta/"
  index_dir: "sources/_index/"
  assets_dir: "sources/_assets/"
  staging_dir: "sources/_staging/"
  scripts_dir: "sources/_scripts/"
...
### Health Checks
- Source count: `ls sources/SOURCE-*.md | wc -l` (should only increase)
- Index integrity: `python anneal.py check-index` (URL index matches filesystem)
- Enrichment coverage: `grep -l 'synopsis: ""' sources/SOURCE-*.md | wc -l` (should decrease over time)
- Stale staging: `ls sources/_staging/` (should be empty between runs)
...
- **URL index (SQLite)** stored at `sources/_meta/source_index.db` (gitignored binary, like ontology.db)
...
- Processed archive moved to `sources/_meta/archives/` for provenance
...
- Original file deleted from watch directory after successful ingest to `sources/`
...
INGEST_SCRIPT="$SYNCRESCENDENCE_ROOT/sources/_scripts/ingest.py"
...
#### URL Index (`sources/_meta/source_index.db` -- SQLite)
...
1. Scan `sources/SOURCE-*.md` for files with `status: raw` or `status: transcript_unavailable`
...
inbox_zero:
  ...
  logging:
    ingest_log: "sources/_meta/INGEST_LOG.md"
    enrichment_log: "sources/_meta/ENRICHMENT_LOG.md"
    error_log: "sources/_meta/INBOX_ZERO_ERRORS.md"
```
...
| Files with `status: raw` | < 25 (one enrichment batch) | `grep -l 'status: raw' sources/SOURCE-*.md \| wc -l` |
| Files with `status: enrich_failed` | 0 (after retry cycles) | `grep -l 'status: enrich_failed' sources/SOURCE-*.md \| wc -l` |
...
- **Storage**: Flat SOURCE-*.md files in `sources/`, flat file dedup by filename
...
| Document | Relevance |
|----------|-----------|
| `canon/sn/CANON-25200` | Task routing decision: "YouTube processing → Gemini" |
| `praxis/syntheses/SYNTHESIS-gemini-cli.md` | Colab-to-Claude bridge pattern for hybrid enrichment |
| `praxis/syntheses/SYNTHESIS-platform_topology_jan2026.md` | Token economics and cost comparison across models |
