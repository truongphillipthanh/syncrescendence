...
**Created**: 2026-01-11
**Authority**: Oracle 12
**Parent**: [[CANON-25000-MEMORY_ARCH-lattice]]-MEMORY_ARCH
...
**Zone Ownership**:
- `sources/processed/a-*`
- `orchestration/logs/*-A.md`
- Stream A processing

**Integration**:
- Reads directives from `orchestration/directives/`
- Writes logs to `orchestration/logs/`
- Updates ledgers in `orchestration/state/`
...
1. **Generate handoff context** using [[CANON-25100-CONTEXT_TRANS-lattice]] protocol
...
- `canon/` — Canonical documents (require Oracle approval)
- `orchestration/oracle_contexts/` — Oracle context files
...
### Formerly CANON-25210: Constellation Architecture Teleology
...
### orchestration/ through praxis/: The Semantic Hierarchy

The numbered directories impose semantic organization on the corpus:

- **orchestration/**: Strategic coordination (state/, scripts/, archive/). The "now" of the system.
- **canon/**: Protected scripture—the foundational documents that define Syncrescendence. 148K words that must not be casually edited. SN-encoded versions in sn/.
- **engine/**: Functions, prompts, avatars, model profiles, queue items. The "how" of the system.
- **sources/**: Source documents (raw/, processed/, research/). The "inputs" to the system.
- **praxis/**: Operational knowledge corpus — synthesis, mechanics, practice patterns, plus memory and exempla.
...
**AVATAR-*.md teleology**: Each platform has a dedicated avatar file in engine/ (e.g., AVATAR-GEMINI-WEB.md, AVATAR-GEMINI-CLI.md, AVATAR-CHATGPT.md) that serves as context preamble. These files orient each platform to its role, the corpus structure, and expected output formats.
...
**END CANON-25200-CONSTELLATION_ARCH-lattice-v2_0.md**
