...
│
├── 00-ORCHESTRATION/           # Strategic coordination (RETAINED)
│   ├── state/                  # ARCH-*, DYN-*, REF-* files
│   ├── scripts/                # dispatch.sh, auto_ingest_loop.sh, etc.
│   └── archive/                # Compacted logs
│
├── 01-CANON/                   # Verified canonical knowledge (PROTECTED, RETAINED)
│
├── 02-ENGINE/                  # Functions, prompts, avatars, templates (RETAINED)
│
├── 04-SOURCES/                 # Source documents (RETAINED)
│
├── 05-SIGMA/                   # Operational knowledge corpus (RETAINED)
│
...
This replaces the ad-hoc pattern where blitzkrieg artifacts, research findings, and multi-agent outputs scattered across `00-ORCHESTRATION/state/impl/`, root directory, and `-OUTGOING/`.
...
```bash
bash 00-ORCHESTRATION/scripts/dispatch.sh psyche "LAUNCHD_MIGRATION" "Update all launchd WatchPaths from -INBOX/<agent>/00-INBOX0 to agents/<agent>/inbox/pending. Reload all services. Verify with launchctl list." "" "DIRECTIVE" "commander"
```

Phase 5 validation:
```bash
bash 00-ORCHESTRATION/scripts/dispatch.sh adjudicator "MIGRATION_VERIFICATION" "Run full kanban lifecycle test on new agent office paths. Dispatch test task, verify pending→active→done flow, verify CONFIRM routing." "" "DIRECTIVE" "commander"
```

---

*Plan authored by Commander (Claude Opus 4.6) | 2026-02-22 | INT-2201 through INT-2204*
*Research corpus consumed: RESEARCH-INSIGHTS-VERY-HIGH-SIGNAL.md, RESEARCH-INSIGHTS-HIGH-SIGNAL.md, 12 source articles from 04-SOURCES/*
