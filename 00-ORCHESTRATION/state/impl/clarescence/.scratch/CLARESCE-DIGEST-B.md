...
- **Git Concurrency — Zone Ownership (Recommended)**: Each agent owns file patterns. Commander owns 00-ORCHESTRATION/state/, 01-CANON/. Adjudicator owns test files, its inbox. Cartographer owns 05-SIGMA/, survey reports. Shared mutable files use append-only semantics or per-agent staging files.
...
- Zone ownership recommendation: Commander owns state/+canon, Adjudicator owns test files, Cartographer owns 05-SIGMA/. Append-only semantics for shared mutable files.
