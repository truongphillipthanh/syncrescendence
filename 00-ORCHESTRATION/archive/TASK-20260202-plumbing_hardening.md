...
- `ops_lint.sh` currently checks **0 files** due to stale assumptions (`02-ENGINE/prompts/specs/registries` don’t exist; 02-ENGINE is flat).
...
1. Patch `00-ORCHESTRATION/scripts/ops_lint.sh` to lint flat `02-ENGINE/PROMPT-*` and `02-ENGINE/REF-*` (so it checks >0 files).
2. Patch `00-ORCHESTRATION/scripts/verify_all.sh`:
   - allow 3 root `.md` files (AGENTS/CLAUDE/COCKPIT)
   - fix directory count computation (use find)
3. Add `00-ORCHESTRATION/scripts/triage_outgoing.sh` (packet lifecycle spotter). Make executable.
