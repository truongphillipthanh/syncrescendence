#!/usr/bin/env python3
"""
URL Index Builder for Syncrescendence 04-SOURCES corpus.

Scans all SOURCE-*.md files, extracts URLs from frontmatter,
writes to 04-SOURCES/_meta/URL_INDEX.txt (one URL per line).

This is the dedup gate for all ingest pipelines.

Usage:
  python build_url_index.py
  python build_url_index.py --sources-dir /path/to/04-SOURCES
"""
...
DEFAULT_SOURCES_DIR = Path(os.environ.get(
    ...
    Path(__file__).resolve().parent.parent.parent / "04-SOURCES"
))
...
def main():
    ...
    parser.add_argument(
        ...
        help=f"Path to 04-SOURCES directory (default: {DEFAULT_SOURCES_DIR})",
    )
