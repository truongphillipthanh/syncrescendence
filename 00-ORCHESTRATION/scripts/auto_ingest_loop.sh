...
extract_reply_to() {
    task_file="$1"
    grep '^\*\*Reply-To\*\*:' "$task_file" 2>/dev/null | head -n1 | sed 's/.*: *//'
}
...
dispatch_task() {
    ...
    if send_prompt "$full_prompt"; then
        log "Dispatched successfully: $task_id"
        return 0
    else
        log "Dispatch failed: $task_id — moving to FAILED"
        mv "$in_progress_path" "${FAILED_DIR}/$(basename "$task_file")"
        echo "DISPATCH_FAILED: Agent rejected or structurally blocked" >> "${FAILED_DIR}/$(basename "$task_file")"
        rm -f "$STATE_FILE"
        return 1
    fi
}
...
check_completion() {
    ...
    if [ -f "$result_file" ]; then
        ...
        mv "$in_progress_path" "${DONE_DIR}/$(basename "$in_progress_path")"
...
CONFIRM_EOF
    ...
    if [ "$elapsed" -gt "$TASK_TIMEOUT" ]; then
        ...
        mv "$in_progress_path" "${FAILED_DIR}/$(basename "$in_progress_path")"
        echo "TIMEOUT: Exceeded ${TASK_TIMEOUT}s" >> "${FAILED_DIR}/$(basename "$in_progress_path")"
        rm -f "$STATE_FILE"
        return 0
    fi
