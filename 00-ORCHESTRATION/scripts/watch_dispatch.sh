...
write_result_receipt() {
  local task_file="$1"; local exit_code="$2"; local output_file="$3"; local fail_reason="${4:-}"
...
pipe_reply_to_sender() {
  local finalized_task="$1"; local exit_code="$2"; local output_file="$3"; local result_path_abs="${4:-}"; local fail_reason="${5:-}"
...
handle_file() {
  ...
  if [ $exit_code -eq 0 ] && [ -f "$out_file" ]; then
    # Phase 1: Resource exhaustion / quota errors (exit code 75)
    if grep -qiE "(you've hit your usage limit|you.ve hit your usage limit|usage limit|rate_limit_exceeded|insufficient_quota|quota exceeded|billing hard limit|exceeded your current quota|RESOURCE_EXHAUSTED|model_capacity_exhausted|ratelimitexceeded|retryablequotaerror|no capacity available for model)" "$out_file" 2>/dev/null; then
      failure_reason=$(grep -oiE "(You've hit your usage limit[^.]*|rate_limit_exceeded|insufficient_quota|quota exceeded|billing hard limit|exceeded your current quota|RESOURCE_EXHAUSTED|model_capacity_exhausted|ratelimitexceeded|retryablequotaerror|no capacity available for model)" "$out_file" 2>/dev/null | head -1)
      log "$(date '+%H:%M:%S') Output validation: RESOURCE EXHAUSTION detected — $failure_reason. Overriding exit code to 75."
      exit_code=75
    # Phase 2: Other fatal model/session/auth errors (exit code 1)
    elif grep -qiE '(does not exist or you do not have access|model .* does not exist|stream disconnected before completion|failed to create session|fatal error:|cannot access session files|permission denied|exec_error|error loading config\\.toml|unknown variant)' "$out_file" 2>/dev/null; then
      failure_reason=$(grep -oiE "(does not exist or you do not have access|model .* does not exist|stream disconnected before completion|failed to create session|fatal error:.*|cannot access session files|permission denied|exec_error|error loading config\.toml|unknown variant)" "$out_file" 2>/dev/null | head -1)
      log "$(date '+%H:%M:%S') Output validation: detected fatal model/session/auth error — $failure_reason. Overriding to FAILED."
      exit_code=1
    fi
  fi
  ...
  local result_abs
  result_abs=$(write_result_receipt "$final_path" "$exit_code" "$out_file" "$failure_reason")
  ...
  pipe_reply_to_sender "$final_path" "$exit_code" "$out_file" "$result_abs" "$failure_reason"
