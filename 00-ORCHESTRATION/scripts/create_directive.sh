...
STAGING="$REPO_ROOT/00-ORCHESTRATION/state/DYN-DIRECTIVE_STAGING.md"
TIMESTAMP=$(date '+%Y-%m-%d %H:%M')
DATE=$(date '+%Y-%m-%d')

# Get next directive number from compendium + staging
LAST_COMPENDIUM=$(grep -oP 'DIRECTIVE-\K[0-9]+' "$REPO_ROOT/00-ORCHESTRATION/archive/ARCH-DIRECTIVE_COMPENDIUM.md" 2>/dev/null | sort -n | tail -1 || echo "048")
