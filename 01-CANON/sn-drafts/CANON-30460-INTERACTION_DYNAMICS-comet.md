...
TERM PatternAConvergentRefinementSamePlatform:
    sutra: "When multiple rounds occur within a single platform:  `` Round 1: Principal → Claude Web → Draft_..."
    gloss:
...
NORM HandoffProtocol:
    sutra: "When the Principal indicates work should transition to another platform: 1"
    gloss:
...
NORM QualityStandards:
    sutra: "- Interpretation should be comprehensive (the Principal's time is the bottleneck) - Artifacts sho..."
    gloss:
...
TERM Phase2PrincipalConfigurationManual:
...
TEST Phase3ValidationHybrid:
    sutra: "Claude Code generates validation scripts; Principal executes cross-platform tests"
    gloss:
