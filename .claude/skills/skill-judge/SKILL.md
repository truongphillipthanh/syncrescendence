---
name: skill-judge
description: "Evaluate Agent Skill design quality against official specifications and best practices. Use when reviewing, auditing, or improving SKILL.md files and skill packages. Provides multi-dimensional scoring and actionable improvement suggestions. provenance: syncrescendence"
---
...
Mandatory gate for the **SKILL_CREATION** pipeline chain within the Syncrescendence constellation:

- **Quality Gate** -- All new skills created within Syncrescendence MUST pass skill-judge evaluation before activation in the constellation. This is a hard gate: no skill enters the active skill registry without a passing grade (C or above, 70%+ score).
- **Pipeline Position** -- skill-judge sits at the terminal position of the SKILL_CREATION chain: ideation -> drafting -> skill-judge -> activation. Failed evaluations loop back to drafting with the improvement report.
- **Adjudicator (CQO) Integration** -- The Adjudicator agent uses skill-judge as its primary evaluation framework for all skill quality assessments. The 8-dimension scoring rubric aligns with the Adjudicator's quality mandate across the constellation.
- **Evaluation Records** -- Skill evaluation reports should be persisted to `00-ORCHESTRATION/state/evaluations/` for audit trail and quality trend tracking across the constellation's skill portfolio.
- **Calibration Standard** -- The Syncrescendence constellation's own white-label skills (this skill included) should periodically be re-evaluated against this rubric to prevent quality drift.
