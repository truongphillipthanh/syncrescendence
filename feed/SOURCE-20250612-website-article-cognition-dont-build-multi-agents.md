...
(Image: Diagram titled "Almost Surely Unreliable" showing hierarchical agent architecture. A "Task" box flows down to an "Agent breaks down task" box (green border), which then splits into two branches: "Subtask 1" leading to "Subagent 1" (blue border) and "Subtask 2" leading to "Subagent 2" (blue border). These then combine through arrows to "Result 1" and "Result 2", flowing to an "Agent combines the results" box (magenta border), which finally produces the "Result".)
...
(Image: Diagram titled "Still Unreliable" showing improved but still problematic architecture. A green "Task" box flows to "Agent breaks down task" which splits into "Subtask 1" and "Subtask 2". Each has a corresponding "Subagent" (1 and 2) in blue boxes. To the right of each subagent are checkboxes showing "Conversation & actions so far" and tracking work on subtasks. Finally, a magenta "Agent combines the results" box combines the outputs with additional checkboxes showing "Combining work".)
...
(Image: Diagram titled "Simple & Reliable" showing sequential linear architecture. A green "Task" box flows down to "Agent breaks down task", which then sequentially processes "Agent does subtask 1", "Agent does subtask 2", and "Agent does subtask 3" in blue boxes. Each step includes checkboxes for "Conversation & actions so far" and tracking individual subtask work. Finally, a magenta "Agent combines the results" box combines the outputs.)
...
(Image: Diagram showing extended sequential architecture with context overflow warning. Similar to previous diagram but extends to show many subtasks with dots indicating continuation, and includes a red dashed box labeled "context overflow" highlighting the problem of excessive subtask work.)
...
(Image: Diagram titled "Reliable on longer tasks (but hard to get right)" showing advanced architecture with context compression. The sequential flow includes an orange "Context Compression LLM" box on the right side that extracts "Key moments & decisions" from the conversation and actions of each subtask agent. This allows the conversation history to be summarized and compressed rather than accumulating infinitely.)
...
[4] [github.com/FoundationAgents/MetaGPT](https://github.com/FoundationAgents/MetaGPT)