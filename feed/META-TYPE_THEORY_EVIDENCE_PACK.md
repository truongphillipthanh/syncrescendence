...
| TE-002 | `canon/sn/METRICS-STREAM-A.md` | **Prefix Inconsistency** | Expected `CANON-` prefix in `canon` namespace. |
| TE-003 | `.claude/skills/readize.md` | **Unbound Instance** | Duplicate of `engine/FUNC-readize.md` without a formal binding (Broken Functor). |
| TE-004 | `engine/PROMPT-GEMINI_CLI_FORENSIC.md` | **Naming Variance** | Internal reference used `PROMPT-GEMINI-CLI-FORENSIC.md` but file is `PROMPT-GEMINI_CLI_FORENSIC.md`. |
...
- **Domain**: Verbose markdown documents in `canon/*.md`
- **Codomain**: Compressed blocks in `canon/sn/*.md`
...
- **Lifts**: A value change in `engine/DEF-CONSTELLATION_VARIABLES.md`
...
- **Intent**: Lift `engine/FUNC-*.md` into `.claude/skills/*.md`.
...
- **Protocol Parametricity**: `engine/FUNC-*.xml` functions are polymorphic but implemented as separate files. They could be unified into a single `FUNC-TRANSFORM<Mode>` where `Mode ∈ {Read, Listen, Integrate}`.
...
1. **Unify Skills**: Delete `.claude/skills/` and symlink to `engine/FUNC-*.md` (requires `claude` CLI support for .md files).
