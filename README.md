...
| **Cartographer** | Exegete | CIO | Gemini CLI (2.5 Pro) | 2 | Mac mini | Pane 4 | "Cartographer, survey..." | **ACTIVE** (Reactivated 2026-02-12) — Resuming corpus intelligence and sensing operations. |
...
- **STAGED**: In agent outbox, ready for commit
...
│   ├── ajna/          # OpenClaw Kimi K2.5 — CSO (MBA remote)
...
- **Communication**: SSH Neural Bridge (bidirectional), Git sync via `agents/ajna/inbox/`
...
|---------|-------|---------|--------|
| Commander Watcher | `com.syncrescendence.watch-commander` | `agents/commander/inbox/pending/` | ACTIVE |
| Adjudicator Watcher | `com.syncrescendence.watch-adjudicator` | `agents/adjudicator/inbox/pending/` | ACTIVE |
| Cartographer Watcher | `com.syncrescendence.watch-cartographer` | `agents/cartographer/inbox/pending/` | ACTIVE |
| Ajna Watcher | `com.syncrescendence.watch-ajna` | `agents/ajna/inbox/pending/` | ACTIVE |
| Psyche Watcher | `com.syncrescendence.watch-psyche` | `agents/psyche/inbox/pending/` | ACTIVE |
...
### Filesystem Kanban (auto_ingest_loop.sh — watch_dispatch.sh DEPRECATED 2026-02-17)
```
inbox/pending/    (PENDING)
  → mv to inbox/active/   (claim + tag agent-hostname)
  → exit 0:   inbox/done/     + CONFIRM + RESULT + EXECLOG
  → exit 124: inbox/blocked/  + timeout notification
  → exit !=0: inbox/failed/   + error report
```
...
3. Validate inbox state:
   - `agents/<agent>/inbox/pending`
   - `agents/<agent>/inbox/active`
...
- Confirm INBOX flow (`inbox/pending → inbox/active → inbox/done / inbox/failed`)

### 5) Emergency Procedures

**All agents down**
- Recreate cockpit (`--kill` then `--launch`), verify watcher labels, restart gateway, then replay pending tasks from `inbox/pending`.
